  ...
  'relative w-full rounded-lg border px-4 py-3 text-sm grid grid-cols-[0_1fr] has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current',
  {
    variants: {
      variant: {
        ...
        destructive: 'border-destructive/30 text-destructive bg-destructive/5 [&>svg]:text-current',
        success: 'border-emerald-500/30 text-emerald-700 bg-emerald-500/5 dark:text-emerald-400',
      },
    },
    defaultVariants: {
      variant: 'default',
