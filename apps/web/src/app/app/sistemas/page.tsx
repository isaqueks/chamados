    ...
    <div className="mx-auto flex max-w-4xl flex-col gap-6">
      <div className="flex items-start justify-between gap-4">
        <div className="flex flex-col gap-1">
          <h1 className="font-heading text-2xl font-semibold tracking-tight">Sistemas-alvo</h1>
        ...
        <CardContent>
          {sistemas.length === 0 ? (
            <div className="flex flex-col items-start gap-3 py-4">
              <p className="text-sm text-muted-foreground">Nenhum sistema-alvo cadastrado ainda.</p>
              <Link
                ...
                <div key={s.id} className="flex items-center gap-3 py-3">
                  <div className="flex min-w-0 flex-1 flex-col gap-1">
                    <div className="flex items-center gap-2">
                      <Link href={`/app/sistemas/${s.id}`} className="font-medium hover:underline">
                        {s.nome}
                      </Link>
                      {!s.ativo && <Badge variant="muted">Inativo</Badge>}
                      {(s.tem_git_credencial || s.tem_logs_credencial || s.tem_bd_credencial) && (
