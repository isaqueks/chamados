...
export function montarSystemPrompt(instrucoesTenant?: string | null): string {
  ...
  return [
    ...
    '- O PDF já sai com a identidade visual da empresa (capa com logo/cor, tabelas estilizadas,',
    '  rodapé paginado) — escreva APENAS o conteúdo em markdown; não descreva estilo nem layout.',
    '- GRÁFICOS (apenas em artefatos PDF — D-027): um bloco de código com linguagem "grafico"',
    '  contendo JSON {"tipo":"barras"|"linhas"|"pizza","titulo":"...","dados":[{"rotulo":"Jan",',
    '  "valor":12}]} vira um gráfico vetorial desenhado no PDF. Use quando números pedirem visual:',
    '  evolução no tempo → linhas; comparação entre categorias → barras; proporção/distribuição →',
    '  pizza (máx. 8 fatias — agregue o resto em "Outros"). Fora do artefato PDF o bloco NÃO é',
    '  suportado (apareceria como código cru) — não o use em respostas nem em md/txt/csv.',
