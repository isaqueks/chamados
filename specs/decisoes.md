...
## D-027 — PDFs de artefato com identidade visual do tenant + gráficos vetoriais nativos (2026-07-27)

**Status:** aceita — evolui o renderer de PDF de D-026 (não muda o contrato `artefato_gerar`).
**Contexto:** avaliação do usuário: os PDFs entregues pela IA estavam "básicos, feios" — o renderer D-026 era pdfkit cru (Helvetica preta, tabela de grade cinza, sem marca). Sugestão inicial: HTML → PDF; restrição firmada na conversa: **nada de chromium** (tem que rodar leve no servidor). LibreOffice headless (DOCX → PDF) foi descartado pelo mesmo motivo (peso comparável ao browser).
**Decisão:** o pdfkit já é uma API de desenho vetorial completa — em vez de trocar de motor, o worker ganha um **template com a identidade visual do tenant** e **gráficos desenhados nativamente**, zero dependência nova:

- **Template branded** (`pdf.ts`): capa com faixa na `cor_primaria` do branding (logo claro do tenant quando PNG/JPEG — carregado LAZY do storage, best-effort — senão `nome_exibicao`) + data de geração; h1/links na cor da marca (escurecida se reprovar contraste 3:1 sobre branco); tabelas com cabeçalho na cor da marca (texto por `melhorTexto`, WCAG), zebra em tint da marca, larguras proporcionais ao conteúdo e colunas 100% numéricas alinhadas à direita; blocos de código com fundo; blockquote com barra lateral; marcadores de lista na cor da marca; rodapé em todas as páginas (`bufferPages`) com nome do tenant + "Página X de Y". Sem branding → paleta padrão do produto (azul-petróleo, D-019). Tints/paleta derivam TODOS da cor da marca (`cores.ts`).
- **Gráficos** (`graficos.ts`): bloco de código com linguagem `grafico` contendo JSON `{tipo: barras|linhas|pizza, titulo?, dados: [{rotulo, valor}]}` vira gráfico vetorial (primitivas do pdfkit): barras/linhas com grade, escala "bonita" (1|2|5×10^k) e rótulos de valor; pizza com fatias em variações da cor da marca e legenda com percentuais. Tetos de legibilidade: 24 pontos (barras/linhas), 8 fatias (pizza — o prompt manda agregar em "Outros"). Spec inválida lança `grafico_invalido:<motivo>` com a forma esperada — erro de ferramenta corrigível pelo modelo, mesmo contrato de D-026.
- **Encanamento:** `contexto.ts` monta a marca (`nome_exibicao`, `cor_primaria` validada, loader lazy do logo com validação de magic bytes) → `montarFerramentasReais` → `criarFerramentaArtefatos`; o download do logo só acontece se a IA de fato gerar um PDF e nunca derruba a triagem. System prompt: anuncia a identidade visual automática ("escreva APENAS o conteúdo") e o bloco `grafico` (linhas para evolução, barras para comparação, pizza para proporção; APENAS em artefatos PDF).
...
**Consequências:** spec 05 §5.6 atualizada; nenhuma dependência nova; testes de validação de spec/paleta + geração real dos três tipos de gráfico e do template (incl. logo corrompido e fallback sem marca). Fica de fora (futuro, se houver demanda): múltiplas séries por gráfico (legenda em barras/linhas agrupadas) e gráficos nas mensagens de chat (hoje o bloco só é suportado no artefato PDF).
