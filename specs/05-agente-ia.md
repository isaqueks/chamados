...
sequenceDiagram
    ...
    participant P as Provider IA (Opus 5)
...
2. **Commit divergente**: o commit da working copy sincronizada em §3.2 é diferente de `conhecimento_commit` (o repositório evoluiu desde o último mapa). A re-geração é **enfileirada** na fila `mapeamento-ia` (D-033) e a triagem que a detectou segue **na hora** com o mapa anterior (defasado, mas útil) — ela não paga os minutos do mapeamento; as triagens seguintes já recebem o mapa novo quando ele concluir.
3. **Manual pelo admin**: ação "Mapear agora" no painel do admin, a qualquer momento — roda fora do contexto de qualquer chamado específico, bastando o `SistemaAlvo` e o commit atual do repositório. É o único gatilho que **ignora** a política por commit abaixo (o admin pode insistir).

**Política por commit (D-033)**: o mapa é função do **commit**, não do chamado. Os gatilhos 1 e 2 disparam **no máximo uma execução automática por commit**: antes de mapear, o worker verifica em `ExecucaoIA` se já existe execução de `gatilho = 'mapeamento'` deste `SistemaAlvo` com o mesmo `commit` no snapshot `entrada` — em **qualquer** status (`executando`/`na_fila` evitam duas em paralelo; `falhou` evita repetir a cada triagem o que já falhou). Existindo, a triagem segue com o mapa armazenado (ou sem mapa) e registra em log que o commit já foi tentado. Motivação: incidente de 2026-09-10 — com o mapeamento falhando por limite de turnos após a troca de modelo, **cada** triagem re-disparava o mapeamento inline (≈3 min e uma execução inteira de Opus 5 por chamado, com custo invisível no registro), porque a falha não deixava rastro consultável.
...
| `IA_MAPA_TURNOS` — máx. turnos/chamadas de ferramenta | corta e conclui com o que tem (mecânica em D-033 abaixo); se nem assim houver resumo, `falhou` com `erro = "max_turnos"`     |
| `IA_MAPA_TIMEOUT_MS` — tempo máx. da exploração       | idem: tenta concluir com o que tem; falhando, `falhou` com `erro = "timeout"`                                                |

**"Corta e conclui com o que tem" (D-033)**: ao atingir o teto de turnos ou o timeout da exploração, o provider **não descarta** o que o modelo já leu. Ele **retoma a mesma sessão** do SDK (`options.resume` com o `session_id` do stream) num **turno de conclusão** sem ferramenta alguma (nativas e MCP desligadas, `maxTurns` mínimo, janela própria de 3 min) e pede o resumo com o que foi levantado — o prompt de conclusão manda dizer em uma linha o que ficou sem investigar, e não inventar. O system prompt do mapeamento também **anuncia o orçamento** (`IA_MAPA_MAX_TURNOS`) e orienta a escrever o resumo antes de esgotá-lo: sem isso, Opus 5 em esforço `high` explorava até estourar o teto sem nunca escrever (20 falhas seguidas em produção, 2026-09-04 a 09-10). Toda falha por limite grava a **telemetria parcial** (tokens somados do stream + custo reportado) na `ExecucaoIA` — o gasto das execuções que falham deixa de ser invisível.

**Defaults (D-033)**: `IA_MAPA_MAX_TURNOS = 100` (40 cabia para Opus 4.8; Opus 5 com esforço `high` usa mais chamadas de ferramenta no mesmo repositório), `IA_MAPA_BUDGET_USD = 10`, `IA_MAPA_TIMEOUT_MS = 600000`, `IA_MAPA_MAX_CHARS` conforme `.env.example`.
...
| `artefato_gerar` _(MCP, D-026)_        | gera um ARQUIVO entregável ao cliente (relatório PDF a partir de markdown — template profissional neutro e gráficos vetoriais via bloco `grafico`, D-027 —, CSV, md, txt), anexado à resposta pública  | tetos de quantidade/tamanho por execução; nome sanitizado com extensão forçada; buffer validado pela MESMA allowlist do upload de usuário; anexado só pelo aplicador (§5.6)                                 |
| `artefato_consulta` _(MCP, D-034)_     | EXTRAÇÃO por consulta: o modelo entrega só o SELECT; o worker o executa (read-only, teto próprio de linhas) e materializa a planilha (`xlsx` ou `csv`) direto do resultado, anexada à resposta pública | mesma validação SELECT-only e mesma conexão read-only de `bd_consultar`; teto `IA_ARTEFATO_MAX_LINHAS`; o modelo recebe só contagem/colunas/amostra, nunca as linhas; demais guardrails de `artefato_gerar` |
...
**Extração por consulta — planilhas CSV/XLSX (D-034)**: `artefato_gerar` exige que o modelo **emita** o conteúdo, o que não escala para extrações de dados: pedidos como "me manda em Excel a relação de todos os clientes da carteira X" levavam o modelo a paginar `bd_consultar` (100 linhas por vez) e a **redigitar milhares de linhas** como texto — quatro timeouts de 10 min no mesmo chamado em produção (2026-09-08 a 09-10). A ferramenta `artefato_consulta` (§4.2) inverte a divisão: o modelo entrega **só o SELECT** (com aliases legíveis, que viram o cabeçalho) e o **worker** executa a consulta — mesma validação SELECT-only, mesma conexão read-only e mesmo timeout de statement de `bd_consultar`, mas com teto próprio de linhas (`IA_ARTEFATO_MAX_LINHAS`, default 10.000) — e materializa o arquivo direto do resultado: `csv` (BOM UTF-8, separador `;` por default — `IA_CSV_SEPARADOR` — para o Excel pt-BR abrir em colunas) ou `xlsx` (gerador OOXML próprio, sem dependência nova: cabeçalho em negrito e congelado, autofiltro, células numéricas tipadas, larguras proporcionais). O modelo recebe de volta **apenas** contagem de linhas, colunas, uma amostra de até 5 linhas e a flag `truncado` (bateu no teto) — para descrever o material em `respostaAoCliente` e avisar quando a lista foi cortada. Entrega, guardrails de nome/quantidade e casos de borda: os mesmos de `artefato_gerar` acima. O system prompt orienta: investigar com `bd_consultar` (amostras, contagens, descobrir tabelas), montar UM SELECT completo e chamar `artefato_consulta`; `xlsx` quando o cliente pedir Excel/planilha. É também a resposta à pergunta "a IA gera relatórios em Excel?": sim, para extrações tabulares.
...
| Chamadas de ferramenta/execução | `IA_MAX_TURNOS` (50)    | triagem: `status = falhou` com `erro = "max_turnos"`, escalona (a saída estruturada não admite "meio JSON"); mapeamento: conclui com o que tem (§3.3, D-033) |

Toda falha por limite (timeout, budget, turnos) grava a **telemetria parcial** — tokens consumidos até o corte e custo reportado pelo provider — na `ExecucaoIA` (D-033); antes essas execuções ficavam com custo nulo e o gasto sumia dos painéis.
...
Fase 1 (RF-17, D-006): **Claude Agent SDK** (programático — controle de ferramentas, structured output, telemetria) com modelo **Opus 5** em esforço de raciocínio **`high`** (D-031; `IA_MODELO`/`IA_ESFORCO`), rodando em worker isolado (infra em `01-arquitetura.md`). O pipeline não referencia diretamente o SDK: fala com uma abstração `AIProvider`, para permitir troca de engine/modelo no futuro sem reescrever a triagem.
