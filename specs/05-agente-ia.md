...
| `artefato_gerar` _(MCP, D-026)_        | gera um ARQUIVO entregável ao cliente (relatório PDF a partir de markdown, CSV, md, txt), anexado à resposta pública     | tetos de quantidade/tamanho por execução; nome sanitizado com extensão forçada; buffer validado pela MESMA allowlist do upload de usuário; anexado só pelo aplicador (§5.6)                 |
