...
| Nome do remetente | `email_remetente_nome` | Campo *From* dos e-mails. | Nome de exibição. |
| E-mail remetente | `email_remetente_endereco` | Endereço *From*. | `no-reply@chamados.app`. |
...
- **Endereço da plataforma** (default): `no-reply@chamados.app` com o `nome_exibicao` do tenant no *From*. Simples, sem configuração de DNS.
