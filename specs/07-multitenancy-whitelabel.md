...
| `ia_instrucoes`                      | text / null    | null                          | Instruções adicionais do admin para a IA do tenant (D-020): tom, contexto do negócio, prioridades, vocabulário. Entram no system prompt de **toda triagem**, numa seção demarcada e **subordinada** às regras da plataforma — nunca relaxam guardrails (separação técnico/cliente, nunca merge/deploy, defesa de injection). Cap de 4.000 caracteres na gravação. Ver `05-agente-ia.md` §4.1.              |
...
| **Repositório git**                  |             |                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `git_repo_url`                       | string      | URL do repositório (https ou ssh) **ou**, quando a instalação define `SISTEMAS_PERMITIR_REPO_LOCAL=true` (default `false`, D-011), um caminho local absoluto ou `file://` — caso típico: self-hosted apontando para código no próprio servidor. Nesse caso a credencial git (`git_credencial_ref`) é dispensável. Em produção com Docker, o diretório precisa estar montado como volume no container do worker. Nota de segurança: `09-seguranca-lgpd.md` §7. |
...
| **Logs**                             |             |                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `logs_tipo`                          | string      | Tipo/fonte de logs. Suportados: `arquivo` (local do worker) e `sftp` (servidor remoto do cliente — D-021). Outros valores (ex.: `cloudwatch`, `loki`) são reservados a adapters futuros e retornam erro claro à IA.                                                                                                                                                                                                                                           |
| `logs_config`                        | jsonb       | Configuração não-secreta de acesso aos logs. `arquivo`: `{ caminho }` (arquivo, diretório ou glob de 1 nível). `sftp` (D-021): `{ host, porta (default 22), usuario, caminho }` — o diretório remoto dos logs é fixado pelo admin; a IA nunca escolhe caminho. Eventuais segredos vão por referência de segredo, nunca em claro.                                                                                                                              |
