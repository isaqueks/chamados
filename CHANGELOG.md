...
- **API `/api/v1`** (nova spec `specs/11-api-mcp.md`): `POST/DELETE /sessao` (login por e-mail+senha → o MESMO token opaco de `Sessao`, revogável, com o rate limit e a resposta anti-enumeração do login da UI), `GET /chamados` (filtros de status/natureza/prioridade/atribuição/busca, keyset), `GET /chamados/{ref}` (chamado + timeline; `{ref}` aceita o NÚMERO do chamado ou o UUID), `POST /chamados/{ref}/mensagens` e `POST /chamados/{ref}/status`.
- **Garantias estruturais:** (a) **Bearer-only** — o cookie do navegador NÃO autentica a API, senão toda rota de escrita seria superfície de CSRF para quem está logado no portal; (b) **a API não é bypass** — mesmos services, mesma `autorizar()`, mesma máquina de estados, mesma RLS: cliente não lê nota interna nem complexidade pela API, como não lê pela UI; (c) corpo de entrada em **markdown** pelo pipeline de sanitização do editor, saída da timeline em **texto puro** (menos tokens, nenhuma tag para o modelo interpretar); (d) mutações passam por `comDespacho`, então notificação e re-enfileiramento de triagem acontecem igual à UI.
- **Servidor MCP** (`apps/mcp`, stdio): ferramentas `chamados_listar`, `chamado_obter`, `chamado_publicar_mensagem`, `chamado_alterar_status`; login preguiçoso com renovação automática em 401; `CHAMADOS_MCP_SOMENTE_LEITURA=true` registra só as de leitura; erro da API volta ao modelo com o código estável (`transicao_invalida` etc.), corrigível. A descrição da ferramenta de mensagem avisa explicitamente que `visibilidade: "publica"` vai AO CLIENTE FINAL.
- **Compartilhado:** `htmlParaTexto` sai de privado do worker para `@chamados/db` (fonte única; ganhou `\t` entre células de tabela e parou de colapsar espaços internos) e novo `resolverIdChamado` (traduz `#12`/UUID → id).
- **Testes:** 22 unitários novos (parsing/projeções da API com a fronteira por papel; cliente MCP com `fetch` fake: sessão preguiçosa, Bearer, renovação em 401 sem laço, senha nunca em mensagem de erro; config) + `npm run smoke:api`, 40 asserts de ponta a ponta sobre HTTP contra a aplicação real.
- **Correção de documentação:** `docs/desenvolvimento.md` e `.env.example` ainda anunciavam `IA_RESOLUCAO_CONFIANCA_MIN=0.7` e resolução automática só para `problema` — defasados desde D-025/D-023 (o código usa `alta` e aceita `alteracao`).

## 2026-07-27 — D-027: PDFs da IA com template profissional neutro + gráficos vetoriais nativos
...
- **Template (`pdf.ts`):** capa com faixa colorida (título + data); títulos/links/marcadores em cor; tabelas com cabeçalho colorido (texto por contraste WCAG), zebra, larguras proporcionais e colunas numéricas à direita; código com fundo; blockquote com barra; rodapé paginado ("Página X de Y") em todas as páginas. Paleta SEMPRE a padrão azul-petróleo do produto (D-019), tints derivados em `cores.ts`.
- **Gráficos (`graficos.ts`):** bloco ` ```grafico ` com JSON `{tipo: barras|linhas|pizza, titulo?, dados: [{rotulo, valor}]}` vira gráfico vetorial desenhado pelo pdfkit — barras/linhas com grade, escala 1|2|5×10^k e valores; pizza com legenda e %. Tetos: 24 pontos, 8 fatias ("Outros"). Spec inválida → `grafico_invalido:<motivo>` com a forma esperada (erro de ferramenta corrigível pelo modelo, como em D-026). System prompt anuncia o layout automático ("escreva APENAS o conteúdo") e o bloco `grafico` (linhas=evolução, barras=comparação, pizza=proporção; só em artefato PDF).
- **Revisão no mesmo dia:** a primeira versão (commit `a9fb0ff`) aplicava a identidade visual do TENANT (cor/logo/nome do `config_branding`) ao PDF; o usuário vetou — **sem branding de tenant no PDF** — e o encanamento da marca foi removido (template neutro sempre).
